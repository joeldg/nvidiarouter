...
- [x] Submit `PARKOUR_ENSEMBLE.md` as a project-scoped draft.
- [x] Obtain human approval and publication; `specreg sync` the bundle.
- [x] Freeze settings/flag names and the panel-membership + max-size semantics.
...
- [x] Add `ENABLE_PARKOUR_ENSEMBLE` (default off), independent of the other
      PARKOUR flags.
- [x] Add `PARKOUR_ENSEMBLE_MODELS` (comma-separated) and
      `PARKOUR_ENSEMBLE_MAX_SIZE`.
- [x] Parse, de-duplicate, and exclude `parkour`; expose the effective panel via
      a settings property. Require >= 2 distinct members or fall back to single
      routing.
- [x] Document settings and the cost/latency multiplication in `.env.example`
      and `README.md`.
...
- [x] Add a server/conductor-controlled `panel` opt-in on `SubtaskSpec`
      (ignored unless the panel is enabled; never client-parameterizable).
- [x] Execute panel members concurrently under the existing scheduler semaphore,
...
- [x] Enforce max panel size (deterministic truncation) and count every member
...
- [x] Tolerate partial failure: proceed on surviving members, record failures,
      fail the node only when all members fail.
- [x] Combine surviving responses: synthesizer by default; when refinement is
      enabled, the verifier MAY select/rank a best candidate.
- [x] Ensure the combination call is routed, bounded, and never selects
      `parkour`.
...
- [x] Aggregate member + combination tokens/cost into the node and PARKOUR
      totals without double counting; retain per-member model/outcome internally.
- [x] Record panel invocations, configured/effective size, member
      successes/failures, distinct models used, added tokens/cost.
- [x] Expose equivalent JSON and Prometheus signals (`nsr_parkour_ensemble_*`).
...
- [x] Adversarial tests: duplicate/invalid members, `parkour` in the list,
...
- [x] Document expected cost/latency multiplication and tuning.
